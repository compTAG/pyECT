...
def weighted_freudenthal(img_arr):
    ...
    higher_simplices = (weighted_edges, weighted_triangles)
...
def freudenthal_triangles(vertices, h, w):
    ...
    u_triangles = torch.stack([tl_vertices, tr_vertices, br_vertices], dim=-1).reshape(-1, 3)
    
    # Lower triangle: top-left, bottom-left, bottom-right
    bl_vertices = get_shifted_vertices(vertices, coords, [1, 0])
    l_triangles = torch.stack([tl_vertices, bl_vertices, br_vertices], dim=-1).reshape(-1, 3)
